...
def get_MNIST_dataloader(opt):
    base_folder = os.path.join(opt.data_input_dir, "MNIST")
    ...
    transform_train = transforms.Compose(
        [get_transforms(eval=False, aug=aug["MNIST"])]
    )
    transform_valid = transforms.Compose(
        [get_transforms(eval=True, aug=aug["MNIST"])]
    )


    # can't get unlabelled
    unsupervised_dataset = torchvision.datasets.MNIST(
        base_folder,
